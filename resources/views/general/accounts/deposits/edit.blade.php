...
<div class="content">
    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title">{{ $deposit->voucher_number }}</h5>
        ...
        <div class="card-body">
            <form action="{{ route('deposit.update', $deposit->id) }}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="deposit_date" class="form-control  @error('deposit_date') is-invalid @enderror" id="anytime-both" value="{{ old('deposit_date', date('d-m-Y g:i:s A', strtotime($deposit->deposit_date))) }}">
                        @error('deposit_date')
                    ...
                    <div class="col-lg-10">
                        <select name="customer_id" class="form-control select-search">
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ (old('customer_id', $deposit->customer->id)== $customer->id) ? 'selected=selected': ''}}>{{ $customer->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-lg-10">
                        <select name="account_id" class="form-control select-search @error('account_id') is-invalid @enderror">
                            ...
                            @foreach($banks as $bank)
                                <option value="{{ $bank->id }}" {{ (old('account_id', $deposit->account->id)== $bank->id) ? 'selected=selected': ''}}>{{ $bank->bank_name }}</option>
                            @endforeach
                        </select>
                        @error('account_id')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="col-lg-10">
                        <select name="purpose_id" class="form-control select-search @error('purpose_id') is-invalid @enderror">
                            ...
                            @foreach($purposes as $purpose)
                                <option value="{{ $purpose->id }}" {{ (old('purpose_id', $deposit->purpose->id)== $purpose->id) ? 'selected=selected': ''}}>{{ $purpose->name }}</option>
                            @endforeach
                        </select>
                        @error('purpose_id')
                            <div class="text-danger">{{ $message }}</div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="money_receipt" class="form-control" placeholder="Money Receipt" value="{{ old('money_receipt', $deposit->money_receipt) }}">
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <div class="input-group">
                            ...
                            <input type="text" name="total_amount" class="form-control @error('total_amount') is-invalid @enderror" placeholder="Total Amount" value="{{ old('total_amount', $deposit->total_amount) }}">
                        </div>
                        @error('total_amount')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="note" class="form-control" placeholder="Note" value="{{ old('note', $deposit->note) }}">
                    </div>
                </div>

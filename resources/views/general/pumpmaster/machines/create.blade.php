    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">New Machine</span>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="card-body">
            <form action="{{ route('machine.store') }}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Machine Name" value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="col-lg-10">
                        <select name="tank_id" class="form-control select-search @error('tank_id') is-invalid @enderror">
                            ...
                            @foreach($tanks as $tank)
                                <option value="{{ $tank->id }}" {{ (old('tank_id')== $tank->id) ? 'selected=selected': ''}}>{{ $tank->name }}</option>
                            @endforeach
                        </select>
                        @error('tank_id')
                            <div class="text-danger">{{ $message }}</div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="model" class="form-control @error('model') is-invalid @enderror" placeholder="Model" value="{{ old('model') }}">
                        @error('model')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <div class="input-group">
                            <input type="number" name="no_of_nozzle" class="form-control" placeholder="No of Nozzle" value="{{ old('no_of_nozzle') }}">
                            @error('no_of_nozzle')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        <input type="text" name="serial_no" class="form-control" placeholder="Serial No" value="{{ old('serial_no') }}">
                        @error('serial_no')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
